...
const fhenixBridgeContract = new ethers.Contract(fhenixBridgeContractAddress, fhenixContractABI, fhenixProvider);
const zamaBridgeContract = new ethers.Contract(zamaBridgeContractAddress, zamaContractABI, zamaWallet);
...
async function main() {
  ...
  fhenixBridgeContract.on('Packet', async (log1, log2, log3, log4, log5) => {
    ...
    const clearTo = `0x${fhenixClient.unseal(fhenixBridgeContractAddress, log3).toString(16)}`;
    ...
    const einput = zamaClient.createEncryptedInput(zamaBridgeContractAddress, signer.address);
